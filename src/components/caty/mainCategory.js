...
import { count_categories, create_category, delete_category, duplicate_category, edit_category, get_category, get_main_categories } from "../../redux/actions/categories";
...
import { COUNT_MAIN_CATEGORY, CREATE_MAIN_CATEGORY, DELETE_MAIN_CATEGORY, DUPLICATE_MAIN_CATEGORY, EDIT_MAIN_CATEGORY, GET_CATEGORY, GET_MAIN_CATEGORIES } from "../../redux/constans/categories"
...
const MainCategory = () => {
    ...
    const { main_categories, category, main_count } = useSelector(state => state.categories)
    ...
    useEffect(() => {
        ...
        dispatch(count_categories(COUNT_MAIN_CATEGORY, { filter: filter.filter }, authorization))
        dispatch(get_main_categories(GET_MAIN_CATEGORIES, { ...filter, skip }))
    }, [dispatch, filter])
    ...
    useEffect(() => {
        setPages((Pages) => {
            Pages.pages.length = Math.ceil(main_count / filter.limit)
    ...
    const duplicateCaty = (category) => {
        dispatch(duplicate_category(DUPLICATE_MAIN_CATEGORY, category, authorization))
    }

    const deleteCaty = (id) => {
        dispatch(delete_category(DELETE_MAIN_CATEGORY, id, authorization))
    }
    ...
    const onSubmit = values => {
        if (params.id && params.id !== "") {
            dispatch(edit_category(EDIT_MAIN_CATEGORY, params.id, values, authorization))
        } else {
            dispatch(create_category(CREATE_MAIN_CATEGORY, values, authorization))
        }
    }
    ...
    return (
        ...
        <div className="ec-content-wrapper">
            <div className="content">
                <Breadcrumb name="Main Categories" />
                {loading && loader()}
                <div className="row">
                                ...
                                <div className="table-responsive">
                                    <div id="responsive-data-table_wrapper" className="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <table id="responsive-data-table" className="table dataTable no-footer" aria-describedby="responsive-data-table_info">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th className="sorting" tabIndex={0} aria-controls="responsive-data-table" rowSpan={1} colSpan={1} aria-label="Sub Categories: activate to sort column ascending" style={{ width: '68.0312px' }}>Sub Categories</th>
                                                    ...
                                                    main_categories.map((category, categoryI) => {
                                                        return (
                                                            <tr key={categoryI} className="odd">
                                                                ...
                                                                <td>
                                                                    <span className="ec-sub-cat-count" title="Total Sub Categories">{category.categories.length}</span>
                                                                    {category.categories && category.categories.length > 0 && category.categories.map((caty, catyI) => {
                                                                        return (
                                                                            <span className="ec-sub-cat-list" key={catyI}>
                                                                                <span className="ec-sub-cat-tag" >
                                                                                    <Link to={`/sub-category/${caty.category._id}`} >
                                                                                        {caty.category.name}</Link>
                                                                                </span>
                                                                            </span>
                                                                        )
                                                                    })}
                                                                ...
                                                                <td>
                                                                    <div className="btn-group">
                                                                        ...
                                                                        <div className="dropdown-menu">
                                                                            <Link className="dropdown-item" to={`/main-category/${category._id}`}>
                                                                                Edit</Link>
                                        ...
                                        <div className="row justify-content-between bottom-information">
                                            <div className="dataTables_info" id="responsive-data-table_info" role="status" aria-live="polite">Showing {(((filter.limit * Pages.currentPage) - filter.limit) + 1)} to {filter.limit * Pages.currentPage} of {main_count} entries</div>
